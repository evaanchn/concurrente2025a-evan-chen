...
`bin/pthreads {folder_with_job}/{job_file_name}`

An example execution command could be: `bin/pthreads jobs/job001b/job001.txt`
...
`bin/pthreads {folder_with_job}/{job_file_name} {thread_count}`

Add a valid amount to the command like so: `bin/pthreads jobs/job001b/job001.txt 10` This way, the simulation will execute with 10 threads.
